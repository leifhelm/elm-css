module PropertiesPhantom exposing (all, allToString)
...
type alias TestCase =
    { functionName : String
    , actualExpectedPairs : List ( String, String )
    }
...
all : Test
all =
    Test.concat (List.map testCaseToTest allTestCases)
...
allToString =
    let
        properties =
            List.concat <| List.map toProperties allTestCases

        toProperties { actualExpectedPairs } =
            List.map Tuple.second actualExpectedPairs
    in
...
testProperty : PropertyDescription -> List ( Style, String ) -> TestCase
testProperty { functionName, property } modifierPairs =
    let
        toString style =
            case toPropertyStrings [ style ] of
                [ str ] ->
                    str
        ...
        constructProperty ( style, expectedStr ) =
            ( toString style, property ++ ":" ++ expectedStr )
    in
    TestCase functionName
        (List.map constructProperty modifierPairs)


testCaseToTest : TestCase -> Test
testCaseToTest { functionName, actualExpectedPairs } =
    Test.describe functionName <|
        List.map expectPropertyWorks actualExpectedPairs


expectPropertyWorks : ( String, String ) -> Test
expectPropertyWorks ( style, expectedStr ) =
    Test.describe expectedStr
        [ Test.test "emitted as expected" <|
            \() ->
                Expect.equal expectedStr style
        ]
